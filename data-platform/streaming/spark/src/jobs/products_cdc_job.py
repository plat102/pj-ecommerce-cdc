"""
Products CDC Job - Processes CDC events from products table
Supports both debug mode (console output) and production mode (ClickHouse)
"""
...
class ProductsCDCJob:
    """Products CDC streaming job"""
    ...
    def create_processed_stream(self):
        ...
        # Read from Kafka
        kafka_df = self.kafka_reader.read_stream("pg.public.products")
        ...
        cdc_df = KafkaMessageParser.parse_json_structures(
            ...
            schemas.get_products_value_schema()
        )
        
        # Transform to target format based on mode
        if self.config.debug_mode:
            # Debug mode: include operation column for debugging
            transformed_df = CDCTransformer.transform_products_cdc_for_debug(cdc_df)
            ...
            transformed_df = CDCTransformer.transform_products_cdc_for_clickhouse(cdc_df)
    ...
    def start_streaming(self):
        ...
        if self.config.debug_mode:
            ...
            self.streaming_query = (self.write_to_console(processed_df)
                                   ...
                                   .option("checkpointLocation", "/tmp/spark-checkpoints/products-debug")
                                   .start())
        else:
            # Production mode: write to ClickHouse using foreachBatch
            batch_writer = self.clickhouse_writer.create_batch_writer_function("products_cdc")
    ...
    def run(self):
        ...
        try:
            logger.info("Starting products CDC job...")
...
def main():
    ...
    # Override app name for products
    config.spark.app_name = "Ecommerce Products CDC Processing"
    
    # Create and run job
    job = ProductsCDCJob(config)
    job.run()
