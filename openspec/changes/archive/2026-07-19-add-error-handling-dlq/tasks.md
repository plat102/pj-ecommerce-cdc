...
- [~] 2.6 Live smoke — **partial**. `ENABLE_SINK_DLQ=1` job started, `docker stop clickhouse` executed, rows produced to `pg.public.products`. Result: Spark batches consumed the offsets but the pre-existing Debezium-Avro → JSON transformer mismatch (Debezium is on the Apicurio Avro converter while `product_cdc_transformer.py` reads `value_json`) drops the rows before the writer runs, so no ClickHouse write is attempted and no sink exception is raised. Sink DLQ code path is thus *not* observably exercised end-to-end by this smoke; the wrapper behavior itself is covered by the 3 unit tests in `tests/spark/test_sink_dlq.py`. Follow-up: bridge the Avro→JSON gap or add a synthetic exception-injection SMT in a later change to close the smoke.
