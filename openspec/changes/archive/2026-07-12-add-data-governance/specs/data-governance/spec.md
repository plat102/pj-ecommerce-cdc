...
> **Status:** All five broad placeholders (`pii-classification`, `ttl-retention`, `schema-contract`, `data-quality-gate`, `lineage-emission`) have been replaced by their 15 phase-scoped requirements below, one section per pillar. See `design.md` § "Requirements That Will Be Declared" for the mapping table.

**Phase 1 — Retention & Lifecycle (decomposed from `ttl-retention`).** The four requirements below are the phase-scoped decomposition of the `ttl-retention` placeholder for Phase 1 (Pillar 4).
